...
export default async function main({ isTypeScript, rootDirectory }) {
  if (!isTypeScript) {
    ...
    throw `Tthis template only supports TypeScript. Please run the command again and select "TypeScript".`;
  }
  ...
  await setupDeployment({ rootDirectory }).catch((error) => {
    ...
    console.error(`Looks like something went wrong setting up deployment.`);
  });
...
async function ensureLoggedIn() {
  ...
  if (loggedInUser) {
    ...
    console.log(
      `You need to login to Fly first. Running \`fly auth login\`...`
    );
